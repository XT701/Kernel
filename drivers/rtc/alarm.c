...
/* Interal RTC timer no this limitation, just keep code align*/
...
#if 0/*Tegra -def CONFIG_SUPPORT_ALARM_POWERON*/
...
#else
			rtc_time_to_tm(rtc_current_time + alarm_wakeup_timer_seconds, &rtc_alarm.time);
			rtc_alarm.enabled = 1;
			rtc_set_alarm(alarm_rtc_dev, &rtc_alarm);
#endif
		} else {
